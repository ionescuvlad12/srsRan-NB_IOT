...
static const uint8_t dummy_sib2_payload[] = {0x00, 0x42, 0x6A, 0xA7, 0x7D, 0xB2, 0xA0, 0x2C, 0xDC,
                                             0xC8, 0xD9, 0x52, 0x28, 0x19, 0x2D, 0x22, 0x9A, 0x13,
                                             0xD2, 0x6A, 0xB8, 0x2A, 0x30, 0x54, 0x20, 0x1D, 0x88,
                                             0x7A, 0x0A, 0x01, 0x61, 0x11, 0x30, 0xDC, 0x0C, 0x5F,
                                             0x22, 0x68, 0x38, 0x24, 0x02, 0x80, 0x20, 0x00, 0x00,
...
int main(int argc, char** argv)
{
  ...
  uint8_t                         sib2_nb_payload[SRSRAN_NPDSCH_MAX_TBS];
  static srsran_npdsch_cfg_t      sib2_npdsch_cfg;
  static srsran_ra_nbiot_dl_dci_t ra_dl_sib2;
  ...
  while ((nf < nof_frames || nof_frames == -1) && !go_exit) {
    for (sf_idx = 0; sf_idx < SRSRAN_NOF_SF_X_FRAME && (nf < nof_frames || nof_frames == -1); sf_idx++) {
      ...
      if (hfn % 2 == 0 && sfn % 512 == 0 && sf_idx == 0) {
        // copy captured SIB2
        memcpy(sib2_nb_payload, dummy_sib2_payload, sizeof(dummy_sib2_payload));

        // configure SIB2-NB parameters

        sib2_params.n                     = 1;
        sib2_params.si_periodicity        = 512;
        sib2_params.si_radio_frame_offset = 0;
        sib2_params.si_repetition_pattern = 4; // Every 4th radio frame
        sib2_params.si_tb                 = 328;
        sib2_params.si_window_length      = 960;
        srsran_nbiot_ue_dl_set_si_params(&ue_dl, SRSRAN_NBIOT_SI_TYPE_SIB2, sib2_params);
      }

      if (send_data && (sfn % 512 == 0 || sib2_npdsch_cfg.sf_idx > 0) && sf_idx != 4 && sf_idx != 0 && sf_idx != 5) {
        ...
        //   srsran_dci_msg_pack_npdsch(&ra_dl_sib2, SRSRAN_DCI_FORMATN1, &dci_msg, false);
        ...
        INFO("%d.%d: Transmitting SIB2-NB.", sfn, sf_idx);
        // configure DL grant for SIB2-NB transmission
        if (sib2_npdsch_cfg.sf_idx == 0) {
          srsran_ra_nbiot_dl_grant_t grant;
          ra_dl_sib2.alloc.i_sf = 6;
          ra_dl_sib2.mcs_idx    = 3;
          ra_dl_sib2.format     = 1;
          srsran_ra_nbiot_dl_dci_to_grant(&ra_dl_sib2, &grant, sfn, sf_idx, DUMMY_R_MAX, true, cell.mode);

          if (srsran_npdsch_cfg(&sib2_npdsch_cfg, cell, &grant, sf_idx)) {
            fprintf(stderr, "Error configuring NPDSCH\n");
            exit(-1);
          }
        }
        ...
        if (SRSRAN_VERBOSE_ISDEBUG()) {
          printf("SIB2-NB payload: ");
          srsran_vec_fprint_byte(stdout, sib2_nb_payload, sizeof(dummy_sib2_payload));
        }

        // Encode SIB2 content
        if (srsran_npdsch_encode_rnti(
                &npdsch, &sib2_npdsch_cfg, &softbuffer, sib2_nb_payload, SRSRAN_SIRNTI, sf_re_symbols)) {
          fprintf(stderr, "Error encoding NPDSCH\n");
          exit(-1);
        }

        if (sib2_npdsch_cfg.sf_idx == sib2_npdsch_cfg.grant.nof_sf || sib2_npdsch_cfg.sf_idx == 0) {
          // srsran_nbiot_ue_dl_get_next_si_sfn(hfn,sfn,sib2_params,&sib2_hfn,&sib2_sfn);
          send_data = false;
          bzero(&sib2_npdsch_cfg, sizeof(srsran_npdsch_cfg_t));
        }
      }
